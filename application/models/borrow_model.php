...
class Borrow_model extends CI_Model{
    ...
    public function getBorrowList($data = array())
    {
        ...
        if (self::IsExist($data['borrow_type']) != false){
        ...
        //判断类型
        if (self::IsExist($data['query_type'])!=false){
            ...
            elseif ($type=="full"){
                if ($data['type']=="repay"){
                    ...
                    $_sql .= " and p1.status = 3 and repay_account_wait=0";
                }elseif (self::IsExist($data['status'])==""){
        ...
        //判断借款状态
        if (self::IsExist($data['status'])!= false){
            if ($data['status']==-1){
        ...
        //判断是否逾期
        if (self::IsExist($data['late_display'])==1 ){
            $_sql .= " and p1.verify_time >".time()." - p1.borrow_valid_time*60*60*24";
        }

        //判断是否担保借款
        if (self::IsExist($data['vouch_status'])!= false){
            $_sql .= " and p1.vouch_status in ({$data['vouch_status']})";
        }


        //判断是体验标
        if (self::IsExist($data['tiyan_status'])!= false){
            $_sql .= " and p1.tiyan_status in ({$data['tiyan_status']})";
        }

        //借款期数
        if (self::IsExist($data['borrow_period'])!= false){
...
//            }
        ...
        //判断年转化率
        if (self::IsExist($data['borrow_apr'])!= false){
            $_sql .= " and p1.borrow_apr=".$data['borrow_apr'];
        }
        if(self::IsExist($data['check_period_valid'])!= false ){

            $_sql .= " and p1.borrow_end_time >".time(). " and p1.borrow_star_time <".time();
        }

        if(self::IsExist($data['is_auto'])!= false ){
        ...
        //借款类别
        if (self::IsExist($data['flag'])!= false){
            $_sql .= " and p1.flag = {$data['flag']}";
        }

        //圈子借款
        if (self::IsExist($data['group_id'])!= false){
        ...
        //借款用途
        if (self::IsExist($data['borrow_use']) != false){
            $_sql .= " and p1.borrow_use in ('{$data['borrow_use']}')";
        }

        //借款用户类型
        if (self::IsExist($data['borrow_usertype']) != false){
            $_sql .= " and p1.borrow_usertype = '{$data['borrow_usertype']}'";
        }

        //是否奖励
        if (self::IsExist($data['award_status'])!= false){
        ...
        //借款
        if (self::IsExist($data['borrow_style']) && $data['borrow_style']!='all' ){
            $_sql .= " and p1.borrow_style in ({$data['borrow_style']})";
        }


        if (self::IsExist($data['account_status']!= false)){
        ...
        if (self::IsExist($data['order'])!= false){
            $order = $data['order'];
            if ($order == "account_up"){
        ...
        //$row = $this->db->getNum($newsql);
        //var_dump($newsql);
        $ret =$this->db->query($newsql);
        $temp= $ret->result_array();
...
//echo $sql1;
        $tmp =$this->db->query($sql1);
        $res =$tmp ->result_array();
        $i=0;
        ...
        $list = array();
            ...
            foreach ($res as $row) {
                if ($data["getDetail"] == 0) {
                    ...
                    $list[$i]['i_qixian'] = iconv('GB2312', 'UTF-8', $this->utility->getBorrowPeriod($row["borrow_period"]));
                    ...
                    //case 26:
                    $list[$i]["t_invest_sum"] = $row["borrow_account_wait"];
                    //case 2:
                    ...
                    //case 30://筹资期限：
                    $borrow_period = $this->utility->getBorrowPeriod($row["borrow_period"]);
                    $list[$i]['t_period'] = $borrow_period;
                    //case 29://还款方式：// 赎回方式
                    $type = $this->utility->getReturntype($row["borrow_style"]);
                    ...
                    $status = $this->utility->getBorrowStatus($star_status, $row["status"], $row);
    ...
    public function GetAttestationsCredit($data = array()){
        ...
        //搜索用户id
        if (self::IsExist($data['type_id'])!=false) {
    ...
    function GetBorrowCount($data){
        ...
        //$_result = $this->db->db_fetch_array($sql);
    ...
    function GetTenderList($data = array()){
        ...
        /*if (IsExist($data['limit'])!=false){
            ...
            return $this->db->db_fetch_arrays(str_replace(array('SELECT', 'SQL', 'ORDER', 'LIMIT'), array($_select, $_sql, $_order, $_limit), $sql));
        }*/
