...
class Raise_model extends CI_Model{
    ...
    public function getRaiseList($data = array())
    {
        ...
        if ($this->IsExist($data['query_id']) != false){
        ...
        //搜索借款名称
        if ($this->IsExist($data['raise_name']) != false){
            $_sql .= " and p1.`raise_name` like '%".urldecode($data['raise_name'])."%'";
        }
        //判断添加时间开始
        if ($this->IsExist($data['dotime1']) != false){
        ...
        //判断添加时间结束
        if ($this->IsExist($data['dotime2'])!=false){
        ...
        //判断借款状态
        if ($this->IsExist($data['raise_type'])!=""){
...
//        $total = intval($row['num']);
        ...
        //分页返回结果
        $data['page'] = !$this->IsExist($data['page'])?1:$data['page'];
        $data['epage'] = !$this->IsExist($data['epage'])?10:$data['epage'];
        ...
        foreach ($res as $row){
            ...
            $status= $this->utility->getRaiseStatus($row->status, $row->end_day);
            $list[$i]["i_status"] = iconv('GB2312', 'UTF-8', $status);
            $list[$i]['invest_amount_yes']= $row->raise_account_yes;
            //raise_period
            $list[$i]['i_period']=iconv('GB2312', 'UTF-8',$row->raise_period."天");
            ...
            $list[$i]['i_endday']=iconv('GB2312', 'UTF-8', $tmp);
