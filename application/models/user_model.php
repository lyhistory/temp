...
class User_model extends  CI_Model
{
    ...
    {
        $sql = "SELECT * FROM yyd_users WHERE email='$email'";
    ...
    {
        $sql = "SELECT * FROM yyd_users WHERE username='$username'";
        $result = $this->fetch_first($sql);
        return $result;
    }
    ...
    public function login($data = array())
    {
        ...
        if (empty($user)) {
            ...
            $user_log["result"] = 0;
            $user_log["content"] = str_replace(array('#keywords#'), array($data['username']), "users_login_error_msg");

            $this->AddUsersLog($user_log);
            $result["status"] = "-2";
        } else {
            $usermane = $user['username'];
            $sql = "select * from yyd_users where username ='$usermane'";
            //echo $sql;
            $temp = $this->db_fetch_array($sql);
            $user_id = $temp['user_id'];

            $data['user_id'] = $user_id;
            $vip = $this->GetUsersVip($data);
            ...
            $verify = $this->getVerifyStatus($data);


            //更新用户的登录信息
            $sql = "update yyd_users set logintime = logintime + 1,up_time=last_time,up_ip=last_ip,last_time='" . time() . "',last_ip='" . $this->ip_address() . "' where username='$username'";
            //echo $sql;
            $this->db->query($sql);
            ...
            $user_log["result"] = 1;
            $user_log["content"] = "users_login_success_msg";
            $this->AddUsersLog($user_log);
    ...
    public function getVerifyStatus($data = array())
    {
        ...
        $resultlist = array();
        $sql = "select status, phone,verify_time from yyd_approve_sms where user_id ='$user_id'";
        ...
        if ($result == false) {
            ...
            $resultlist['phone_verify_time'] = date('Y-m-d H:i:s', $result['verify_time']);
        }
        ...
        if ($result == false) {
            ...
            $resultlist['realname_name'] = iconv('GB2312', 'UTF-8', $result['realname']);
    ...
    function CheckEmail($data = array())
    {
        ...
        $sql = "select 1 from yyd_users where  email = '{$data['email']}' $_sql";
        $result = $this->db_fetch_array($sql);
        //如果邮箱不存在的话则返回
    ...
    function CheckPhone($data = array())
    {
        ...
        $sql = "select 1 from yyd_users where  phone = '{$data['phone']}' $_sql";
        $result = $this->db_fetch_array($sql);
        //如果phone不存在的话则返回
        if ($result == false) return false;
        return true;
    ...
    function CheckUsername($data = array())
    {
        ...
        $sql = "select 1 from yyd_users where  username = '{$data['username']}' $_sql";
        $result = $this->db_fetch_array($sql);
        //如果用户名不存在的话则返回
...
//        }
        ...
        if (!$result) {
            ...
            $sql = "insert into yyd_users_info set user_id='{$user_id}',type_id={$result_users_type['id']},status =1";
            $this->db->query($sql);
    ...
    public function setDepositPwd($data)
    {
        ...
        $pwd = md5($pwd);
        $reply = 0;
        $result = $this->db->query("Update yyd_users SET  paypassword='$pwd' where user_id='$userid'");
        if (!$result) {
            $reply = -1;
        } else {
            $reply = 1;
    ...
    function getPwd($data = array())
    {
        ...
        if (!$user) {
            ...
            $result = $this->db->query("Update yyd_users SET  password='$newpwd' where  username='$username'");
            if (!$result) {
    ...
    function sendEmail($data = array())
    {
        ...
        $this->db->query("insert into yyd_users_email_log set email='{$email}',send_email='{$send_email}',user_id='{$user_id}',title='{$title}',msg='{$msg}',type='{$type}',status='{$status}',addtime='" . time() . "',addip='" . $this->User_model->ip_address() . "'");
    ...
    public function sendoutEmail($subject, $body, $to, $data)
    {
        ...
        $this->email->from($data['con_email_from'], iconv('GB2312', 'UTF-8', $data['con_email_from_name']));
        $this->email->to($to);
        $this->email->subject(iconv('GB2312', 'UTF-8', $subject));
        $this->email->message(iconv('GB2312', 'UTF-8', $body));
        $this->email->send();
    ...
    function UpdateUsersInfo($data = array())
    {
        ...
        $this->db->query($sql . join(",", $_sql) . " where user_id='{$data['user_id']}' ");
        return $data['user_id'];
    }
    ...
    function CountCredit($data)
    {
        if ($data['type'] == "catoreasy") {
            ...
            $result = $this->Borrow_model->GetBorrowCredit(array("user_id" => $data['user_id']));
        ...
        if ($input["type"] == "phone") {
            if ($_SESSION['smscode_time'] + 60 > time() && $_SESSION['smscode_phone'] == $input['phone']) {
                ...
                if ($isExist != false) {
                    ...
                    /*
                    if($result=="approve_sms_phone_status_exiest"){
                        echo "1";
                        exit;
                    }*/
    ...
    function UpdateEmail($data = array())
    {
        ...
        if ($result == false) {
            ...
            $admin_log["result"] = 0;
            $admin_log["content"] = $data["username"] . "users_update_email_error_msg";
            $admin_log["data"] = $data;
    ...
    public function getBankCard($user_id){
        ...
        $sql = "select *  from yyd_user_bank where user_id = ".$user_id;
            ...
            if ($msg==""){
                ...
                if ($msg==""){
                    ...
                    if ($data['type']==1){
                        ...
                        $data['remark'] = $params['payname']."在线充值";
